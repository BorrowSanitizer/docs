...
Soon afterward, [@cmazakas](https://github.com/cmazakas) pointed us toward `fiona-rs`, a new encapsulation for the `io_uring` system call interface. We found another aliasing bug in this library. It had a similar pattern to the first one: 
```Rust
#[derive(Clone)]
pub struct Executor {
    p: Rc<IoContextFrame>,
...
You can try running BorrowSanitizer on your library today using our [container image](https://github.com/BorrowSanitizer/bsan/pkgs/container/bsan). Once inside the container, it's as simple as cloning your library and running the following command:
```
cargo bsan test
```
### Conclusion
